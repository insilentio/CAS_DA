...
corrplot(mcor, addCoef.col = "white", number.cex = .7)
...
ggpairs(c2009[,2:5],diag=list(continuous = "barDiag"))
ggpairs(c2009[,2:5],diag=list(continuous = "barDiag"),lower = list(continuous = "smooth"))

# Zwischen welchen Variablen ist der Zusammenhang am stärksten?
# ist der Zusammenhang überall linear?
#zw. GDP und health exp
...
profil_cars <- mtcars %>%
  ...
  mutate_each(funs(rescale), -car) %>%  # Reskalieren der Variablen zwecks Vergleichbarkeit (alle zw. 0 und 1)
